...
export interface CallViewModel {
  ...
  /** Whether the user has pinned the standard header to remain visible. */
  headerPinned$: Behavior<boolean>;
  /** Toggle the user's standard-header visibility preference. */
...
export function createCallViewModel$(
  ...
  const headerPinnedSubject$ = new BehaviorSubject(true);
  const headerPinned$ = scope.behavior(headerPinnedSubject$);
  const toggleHeaderPinned = (): void => {
    headerPinnedSubject$.next(!headerPinnedSubject$.value);
  };

  const automaticallyShowHeader$ = scope.behavior<boolean>(
    windowMode$.pipe(
      switchMap((mode) => {
  ...
  const showHeader$ = scope.behavior(
    combineLatest([automaticallyShowHeader$, headerPinned$]).pipe(
      map(([automaticallyShow, pinned]) => automaticallyShow && pinned),
    ),
  );
